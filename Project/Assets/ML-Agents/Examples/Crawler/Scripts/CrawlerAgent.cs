...
public class CrawlerAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        {
            // Decrease the reward if the agent collides with the wall
            AddReward(-1f);

            // Take action to avoid the wall
            TakeActionToAvoidWall();
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        {
            // Decrease the reward if the agent collides with the wall
            AddReward(-0.5f); // Adjust the penalty as needed
        }
    }
    ...
    // Method to check if the agent is colliding with the wall
    ...
    void TakeActionToAvoidWall()
    {
        // Calculate a new target direction away from the wall
        Vector3 awayFromWallDirection = CalculateDirectionAwayFromWall();

        // Rotate the agent to face the new direction
        RotateAgentTowardsDirection(awayFromWallDirection);
    }

    Vector3 CalculateDirectionAwayFromWall()
    {
        // Cast rays in different directions and find the direction that has the least number of collisions
        List<Vector3> directions = new List<Vector3>() { Vector3.forward, Vector3.right, Vector3.back, Vector3.left }; // Add more directions if needed

        int minCollisions = int.MaxValue;
        Vector3 awayFromWallDirection = Vector3.zero;

        foreach (Vector3 direction in directions)
        {
            int collisions = CountCollisionsInDirection(direction);
            if (collisions < minCollisions)
            {
                minCollisions = collisions;
                awayFromWallDirection = direction;
            }
        }

        return awayFromWallDirection.normalized;
    }

    int CountCollisionsInDirection(Vector3 direction)
    {
        int collisions = 0;
        RaycastHit[] hits = Physics.RaycastAll(body.position, direction, 10f);
    ...
    void RotateAgentTowardsDirection(Vector3 direction)
    {
        Quaternion targetRotation = Quaternion.LookRotation(direction, Vector3.up);
        body.rotation = Quaternion.RotateTowards(body.rotation, targetRotation, 90f);
    }
